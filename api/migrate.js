...
export default async function handler(req, res) {
  ...
  const stats = {
    rideKeysScanned: 0, rideKeysMigrated: 0, rideReadingsCopied: 0,
    crowdKeysScanned: 0, crowdKeysMigrated: 0, crowdReadingsCopied: 0,
    errors: [],
  };

  try {
    // ── 1. Scan all old wt: keys ────────────────────────────────────────────
    // Old format: wt:<uuid>:<dow>:<hour>  (4 segments after wt:)
    // New format: wt:<uuid>:<season>:<dow>:<hour>  (5 segments)
    // We identify old keys by checking they have exactly 4 colon-separated parts
    // after the prefix.
    ...
    do {
      ...
      cursor   = Array.isArray(result) ? result[0] : '0';
      const keys = Array.isArray(result) ? result[1] : [];
      // Old format has exactly 3 colons after "wt:" → total 4 parts split by ":"
      // e.g. wt:uuid:2:14  → ['wt', 'uuid', '2', '14'] — but uuid itself has hyphens not colons
      // Actually: wt:75ea578a-adc8-...:2:14 → split(':') gives more than 4 due to uuid hyphens
      // Better check: new keys have a season word in them
      const oldKeys = keys.filter(k => {
        const parts = k.split(':');
        // New keys: wt:<id>:<season>:<dow>:<hour> where season is one of value/holiday/summer/peak
        // Old keys: wt:<id>:<dow>:<hour> where dow is 0-6
        // The segment after the UUID (parts[2] for simple IDs, but UUIDs have hyphens not colons)
        // Since UUID uses hyphens, the split gives: ['wt', '<full-uuid>', '<dow-or-season>', '<hour-or-dow>', '<hour-if-new>']
        // Old: 4 parts total. New: 5 parts total.
        return parts.length === 4 && !SEASONS.includes(parts[2]);
      });
      rideKeys = rideKeys.concat(oldKeys);
    } while (cursor !== '0');

    stats.rideKeysScanned = rideKeys.length;

    // Migrate ride keys in batches
    const BATCH = 50;
    for (let i = 0; i < rideKeys.length; i += BATCH) {
      const batch = rideKeys.slice(i, i + BATCH);
      // Read all values
      ...
      readResults.forEach(({ result }, idx) => {
        ...
        const key   = batch[idx];
        const parts = key.split(':');
        // parts: ['wt', '<uuid>', '<dow>', '<hour>']
        const rideId = parts[1];
        const dow    = parts[2];
        const hour   = parts[3];

        // Copy into every season bucket — distribute evenly
        for (const season of SEASONS) {
          const newKey = `wt:${rideId}:${season}:${dow}:${hour}`;
          for (const val of result) {
            writeCmds.push(['LPUSH', newKey, String(val)]);
          }
          writeCmds.push(['LTRIM', newKey, '0', String(MAX_SLOT - 1)]);
        }

        stats.rideKeysMigrated++;
        stats.rideReadingsCopied += result.length * SEASONS.length;
      });

      if (writeCmds.length) await redisPipeline(writeCmds);
    }

    // ── 2. Scan all old crowd: keys ─────────────────────────────────────────
    let crowdKeys = [];
    cursor = '0';
    do {
      const result = await redisCmd(['SCAN', cursor, 'MATCH', 'crowd:*', 'COUNT', '200']);
      ...
      const oldKeys = keys.filter(k => {
        const parts = k.split(':');
        // Old: crowd:<parkId>:<dow>:<hour> → 4 parts
        // New: crowd:<parkId>:<season>:<dow>:<hour> → 5 parts
        return parts.length === 4 && !SEASONS.includes(parts[2]);
      });
      crowdKeys = crowdKeys.concat(oldKeys);
    } while (cursor !== '0');

    stats.crowdKeysScanned = crowdKeys.length;
    ...
    for (let i = 0; i < crowdKeys.length; i += BATCH) {
      ...
      readResults.forEach(({ result }, idx) => {
        ...
        const key    = batch[idx];
        const parts  = key.split(':');
        // parts: ['crowd', '<parkId>', '<dow>', '<hour>']
        const parkId = parts[1];
        const dow    = parts[2];
        const hour   = parts[3];

        for (const season of SEASONS) {
          const newKey = `crowd:${parkId}:${season}:${dow}:${hour}`;
          for (const val of result) {
            writeCmds.push(['LPUSH', newKey, String(val)]);
          }
          writeCmds.push(['LTRIM', newKey, '0', String(MAX_SLOT - 1)]);
        ...
        stats.crowdKeysMigrated++;
        stats.crowdReadingsCopied += result.length * SEASONS.length;
      });
